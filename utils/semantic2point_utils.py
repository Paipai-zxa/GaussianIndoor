...
def create_point_cloud(color_path, depth_path, intrinsic, extrinsic, downsample_factor=8, scene=None, convert_to_color=False):
    ...
    color_small = Image.fromarray(color).resize((new_w, new_h), Image.NEAREST)
...
def process_scene_point_clouds(scene_dir, semantic_dir, output_dir, is_depth_anything=False, convert_to_color=False):
    ...
    for color in colors_list:
        ...
        pcd = create_point_cloud(color_path, depth_path, intrinsic, extrinsic, scene=os.path.basename(scene_dir), convert_to_color=convert_to_color)
