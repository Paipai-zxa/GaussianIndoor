...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : List[Camera], spatial_lr_scale : float):
    ...
    def training_setup(self, training_args):
        ...
        self.offset_gradient_accum = torch.zeros((self.get_xyz.shape[0]*self.n_offsets, 1), device="cuda")
        self.offset_denom = torch.zeros((self.get_xyz.shape[0]*self.n_offsets, 1), device="cuda")
        self.anchor_demon = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
    ...
    def anchor_growing(self, grads, threshold, offset_mask):
        ...
        for i in range(self.update_depth):
            ...
            all_xyz = self.get_xyz.unsqueeze(dim=1) + self._offset * self.get_scaling[:,:3].unsqueeze(dim=1)
    ...
    def adjust_anchor(self, check_interval=100, success_threshold=0.8, grad_threshold=0.0002, min_opacity=0.005):
        ...
        padding_offset_demon = torch.zeros([self.get_xyz.shape[0]*self.n_offsets - self.offset_denom.shape[0], 1],
                                           dtype=torch.int32, 
                                           device=self.offset_denom.device)
        self.offset_denom = torch.cat([self.offset_denom, padding_offset_demon], dim=0)

        self.offset_gradient_accum[offset_mask] = 0
        padding_offset_gradient_accum = torch.zeros([self.get_xyz.shape[0]*self.n_offsets - self.offset_gradient_accum.shape[0], 1],
                                           dtype=torch.int32, 
