...
from pre_process.scene_config import get_scene_config
import json
...
def generate_mask(img_name, args, dino_model, sam_predictor, config, id2label, Thing_sem, Segment_sem):
    ...
    for i in range(transformed_boxes.shape[0]):
        ...
        if (transformed_boxes[i][2].item()-transformed_boxes[i][0].item())>W*0.7 and (transformed_boxes[i][3].item()-transformed_boxes[i][1].item())>H*0.7:
            ...
            if phrases[i] in config["class_name"]:
                ...
                Segment_sem[img_name].append(class_label)
                # THING or STUFF
    ...
    cv2.imwrite(os.path.join(folder_semanitc+'_rgb', f"{img_name.replace('.jpg', '.png').replace('JPG', 'png')}"), frame_with_semantic)
    cv2.imwrite(os.path.join(folder_semanitc, f"{img_name.replace('.jpg', '.png').replace('JPG', 'png')}"), semantic_label.astype(np.int8))
    
    cv2.imwrite(os.path.join(folder_instance+'_rgb', f"{img_name.replace('.jpg', '.png').replace('JPG', 'png')}"), frame_with_instance)
    cv2.imwrite(os.path.join(folder_instance, f"{img_name.replace('.jpg', '.png').replace('JPG', 'png')}"), instance_label.astype(np.int8))
...
def parse_args():
    ...
    parser.add_argument('--dino_config', type=str, default="submodules/GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py", help='GroundingDINO config file path')
    parser.add_argument('--dino_model', type=str, default="submodules/GroundingDINO/groundingdino_swint_ogc.pth", help='GroundingDINO model weights path')
    parser.add_argument('--sam_checkpoint', type=str, default='submodules/segment_anything/sam_vit_h_4b8939.pth', help='SAM model checkpoint path')
...
if __name__ == "__main__":
    ...
    images = sorted(images, key=lambda x: int(x.replace("DSC", "").split('.')[0]))
    Thing_sem = {}  # For instance images
    Segment_sem = {}
    
    for img_name in tqdm(images):
        Thing_sem[img_name] = []
        Segment_sem[img_name] = []
        generate_mask(
            img_name, 
            args, 
            dino_model, 
