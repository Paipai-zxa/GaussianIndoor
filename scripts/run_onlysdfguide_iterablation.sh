...
base_exp_name=train_den_0.95_prune_0.2
...
# pruning_threshold_values=(0.02 0.1 0.2 0.5)
# densification_threshold_values=(0.95 0.98 0.99)
...
for sdf_guidance_start_iter in "${sdf_guidance_start_iter_values[@]}"; do
    for sdf_guidance_end_iter in "${sdf_guidance_end_iter_values[@]}"; do
        for sdf_guidance_interval in "${sdf_guidance_interval_values[@]}"; do
            exp_name="${base_exp_name}_StartIter_${sdf_guidance_start_iter}_EndIter_${sdf_guidance_end_iter}_Interval_${sdf_guidance_interval}_${current_time}"
            output_path=output/${dataset_name}/${scene}/run_onlysdfguide_iterablation/${exp_name}
            mkdir -p ${output_path}
