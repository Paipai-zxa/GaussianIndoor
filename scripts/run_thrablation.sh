...
pruning_threshold_values=(0.02 0.1 0.2 0.5 0.7)
...
for densification_threshold in "${densification_threshold_values[@]}"; do
    for grad_sdf_omega in "${grad_sdf_omega_values[@]}"; do
        for pruning_threshold in "${pruning_threshold_values[@]}"; do
            ...
            exp_name="${base_exp_name}_Dens_${densification_threshold}_Prun_${pruning_threshold}_Omega_${grad_sdf_omega}_${current_time}"
            output_path=output/${dataset_name}/${scene}/run_thrablation/${exp_name}
            mkdir -p ${output_path}
