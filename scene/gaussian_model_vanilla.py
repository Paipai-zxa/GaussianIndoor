...
class  GaussianModel:
    ...
    def __init__(self, 
        ...
        if self.enable_geo_mlp:
            ...
            self.geo_mlp_cov = nn.Sequential(
                nn.Linear(3+self.cov_dist_dim+shs_feature_dim, feat_dim),
                nn.ReLU(True),
    ...
    def training_setup(self, training_args):
        ...
        if self.enable_geo_mlp:
            self.geo_mlp_cov_scheduler_args = get_expon_lr_func(lr_init=training_args.geo_mlp_lr_init,
                                                        ...
                                                        max_steps=training_args.geo_mlp_lr_max_steps)
        if self.use_feat_bank:
    ...
    def _prune_optimizer(self, mask):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            if group["name"] == "geo_mlp_cov" or group["name"] == "mlp_featurebank":
                continue
            stored_state = self.optimizer.state.get(group['params'][0], None)
