...
def clean_mesh_faces_outside_frustum(path_save_clean, path_mesh, 
    ...
    logging.info(f'Surfaces/Kept: {len(mesh.faces)}/{len(values[1:])}')
    ...
    if len(mesh_o3d.vertices) == 0:
        logging.error(f'Cleaned mesh has no vertices. Using original mesh instead.')
        # 如果清理后的网格没有顶点，使用原始网格
        mesh_o3d = read_triangle_mesh(path_mesh)
        # 复制原始网格到目标路径
