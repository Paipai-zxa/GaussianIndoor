...
#     """
#     height, width = depth_image.shape
#     reprojected_depth = np.zeros_like(depth_image, dtype=np.float32)
...
#             # 将3D坐标投影到颜色图坐标系
#             u_color = int((K_color[0, 0] * X / Z) + K_color[0, 2])
#             v_color = int((K_color[1, 1] * Y / Z) + K_color[1, 2])
            
#             if 0 <= u_color < width and 0 <= v_color < height:
#                 reprojected_depth[v_color, u_color] = depth  # 更新重投影后的深度值

#     reprojected_depth = reprojected_depth.astype(np.uint16)
...
# scene_name = depth_path.split("/")[5]
# t = tqdm.tqdm(total=len(depth_files), desc=f"Scene: {scene_name}")
# for depth_file in depth_files:
#     depth_image = cv2.imread(os.path.join(depth_path, depth_file), cv2.IMREAD_UNCHANGED)
#     K_depth = np.loadtxt(depth_intrinsics_path).reshape(4, 4)
#     K_color = np.loadtxt(color_intrinsics_path).reshape(4, 4)

#     reprojected_depth = reproject_depth_to_color(depth_image, K_depth, K_color)
#     # 保存为npy格式
#     np.save(os.path.join(args.data_path, "reprojected_depths", depth_file.replace(".png", ".npy")), reprojected_depth)
#     t.update(1)
# t.close()