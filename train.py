        ...
        with torch.no_grad():
            ...
            if dataset.enable_scaffold:
                ...
                if iteration < opt.densify_until_iter:
                    ...
                    if opt.densify_from_iter < iteration < opt.sdf_guidance_start_iter and iteration % opt.densification_interval == 0 and gaussians.get_xyz.shape[0] < 2000000:
                        size_threshold = 20 if iteration > opt.opacity_reset_interval else None
                        gaussians.densify_and_prune(opt.densify_grad_threshold, 0.005, scene.cameras_extent, size_threshold, radii)

                    if opt.sdf_guidance_start_iter <= iteration < opt.sdf_guidance_end_iter and iteration % opt.sdf_guidance_interval == 0 and gaussians.get_xyz.shape[0] < 2000000:
                        size_threshold = 20 if iteration > opt.opacity_reset_interval else None
