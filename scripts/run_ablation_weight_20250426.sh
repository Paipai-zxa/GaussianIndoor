...
plane_constraint_weight_values=(0.015 0.15 1.5)
...
for plane_weight in "${plane_constraint_weight_values[@]}"; do
    for scale_weight in "${scale_flatten_weight_values[@]}"; do
        exp_name="${base_exp_name}_planeWeight_${plane_weight}_scaleWeight_${scale_weight}_${current_time}"
        output_path=output/${dataset_name}/${scene}/${exp_name}
        mkdir -p ${output_path}

        # 构建命令
        command="python train.py -s data/${scene} -m ${output_path} --use_depth_regularization --depth_l1_weight_init 1000000 --depth_l1_weight_final 100000 --use_plane_constraint --plane_constraint_iteration 7000 --plane_constraint_weight ${plane_weight} --use_scale_flatten --scale_flatten_iteration 0 --scale_flatten_weight ${scale_weight} --enable_geo_mlp --opt_geo_mlp_iteration 7000 --feat_dim 32 --iterations ${iterations} --eval"
